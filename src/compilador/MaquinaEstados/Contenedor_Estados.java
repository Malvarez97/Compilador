package compilador.MaquinaEstados;


public class Contenedor_Estados {
public static final int INICIAL = 0;
public static final int DETECCION_ID = 1;
public static final int DETECCION_PR = 2;
public static final int COMENTARIO = 3;
public static final int COMENTARIO2= 4;
public static final int COMENTARIO3= 5;
public static final int COMPYMAYOR = 6;
public static final int DISTYMENOR = 7;
public static final int AND = 8;
public static final int OR = 9;
public static final int ASIGNACION = 10;
public static final int UNALINEA = 11;
public static final int D_U_1 = 12;
public static final int D_U_2 = 13;
public static final int D_U_3 = 14;
public static final int D_U_4 = 15;
public static final int D_U_5= 16;
public static final int D_U_6=17;
public static final int FINAL = 18;

public static final int TOTAL_ESTADOS = 19;
}
