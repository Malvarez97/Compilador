    ...
    public class TablaSimbolos {
        ...
        public String toAsm(){
            ...
            for (String lexema : tablaSimb.keySet()) {
                Casilla casilla = tablaSimb.get(lexema);
                ...
                //DOUBLE
                if (casilla.getTipo().equals(casilla.TIPO_DOUBLE)){
                    if (casilla.esCte())
                ...
                //UINT
                if (casilla.getTipo().equals(casilla.TIPO_UINT)
                        && (casilla.getUso().equals(casilla.USO_VAR) || casilla.isParamCVR() || casilla.isParamCV())){
                    if (lexema.startsWith("PROGRAM")) asmBuilder.append('_'); //Variable no auxiliar.
        ...
        public void agregarReferencia(String lexema){
            Casilla casilla = tablaSimb.get(lexema);
            casilla.actualizarReferencias(1); //refs++
        }

        public void quitarReferencia(String lexema) {
            Casilla casilla = tablaSimb.get(lexema);
            casilla.actualizarReferencias(-1); //refs--
            if (casilla.sinReferencias()) tablaSimb.remove(casilla.getLexema());
        }
